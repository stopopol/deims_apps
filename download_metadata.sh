...
for code in `jq .[].id.suffix deims_sites.json | sed "s/\"//g"`; do  echo $code; wget "https://pydeims.dev.klimeto.com/gmd/site?uuid=${code}" -O ${code}_site.xml; done

wget  -O  deims_datasets.json https://deims.org/api/datasets
for code in `jq .[].id.suffix deims_datasets.json | sed "s/\"//g"`; do  echo $code; wget "https://pydeims.dev.klimeto.com/gmd/dataset?uuid=${code}" -O ${code}_dataset.xml; done
