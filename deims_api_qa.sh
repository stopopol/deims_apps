...
wget  -O  deims_locations.json https://deims.org/api/locations
for code in `jq .[].id.suffix deims_locations.json | sed "s/\"//g"`; do  echo $code; wget "https://deims.org/api/locations/${code}"; echo $?; done > downloadlist_locations.txt
cat downloadlist_locations.txt | egrep -B1 "^8$" > faulty_locations.txt
